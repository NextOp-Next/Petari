...
TalkBalloon::TalkBalloon(const char *pArg) :
    LayoutActor(pArg, true),
    _20(nullptr),
    _24(0),
    _28(false),
    _29(0)
...
TalkBalloonShort::TalkBalloonShort(const char *pArg) : 
    TalkBalloon(pArg),
    _2C(0)
{
...
void TalkBalloonShort::exeTalk() {
    ...
    if (_2C != 0) {
        if (MR::isStep(this, _2C)) {
            ...
            if (MR::isGreaterStep(this, _2C + 30)) {
                open(_20);
            }
        }
    }
...
TalkBalloonEvent::TalkBalloonEvent(const char *pArg) :
    TalkBalloon(pArg),
    _2C(1),
    _30(0)
{
...
void TalkBalloonEvent::exeTalk() {
    ...
    if (_24->isTextAppearedAll() && MR::isDead(_30) && !TalkFunction::getMessageInfo(_20)->isBalloonFix()) {
        if (!TalkFunction::isSelectTalk(_20) || turnPage()) {
...
TalkBalloonInfo::TalkBalloonInfo(const char *pArg) :
    TalkBalloon(pArg)
{
...
TalkBalloonIcon::TalkBalloonIcon(const char *pArg) :
    TalkBalloonShort(pArg)
{
...
TalkBalloonHolder::TalkBalloonHolder() :
    _14(0)
{
